with cust as (
    select
        ...
        cca.ccfname,
        cca.cclname
    from {{ ref('stg_dremio_bscs_customer_all') }} as cua
    left join {{ ref('stg_dremio_bscs_contract_all') }} as coa on cua.customer_id = coa.customer_id
    left join {{ ref('stg_dremio_bscs_ccontact_all') }} as cca on cua.customer_id = cca.customer_id
    where 1=1
    and cca.ccseq = '1'
)
, serv as (
    ...
    from {{ ref('stg_dremio_bscs_contr_services_cap') }}
)
select
